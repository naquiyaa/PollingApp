...
const createPoll = async (req, res) => {
  ...
  console.log("req.body: ", req.body.options);
  ...
  try {
    ...
    const { optimizedImageBuffer, initialSize, finalSize } = await optimizeImageWithTinyPNG(imageBuffer);
    ...
    const validOptions = req.body.options.filter(option => typeof option.text === 'string' && option.text.trim() !== '');
    ...
    res.status(201).json({
      ...
      poll,
      initialSize,  // Initial size in bytes
      finalSize,    // Final size in bytes
    });
  } catch (error) {
    console.error(error);
...
const updatePoll = async (req, res) => {
    const { pollId, question, options, image } = req.body;
